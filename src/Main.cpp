...
void Grid::draw()
{
    ...
    for (int row = 0; row < Number_of_rows; row++)
    {
        ...
        for (int column = 0; column < Number_of_columns; column++)
        {
            ...
            DrawRectangle((column * (cell_size + 15)) + 1 /*x-cordinate*/, row * cell_size /*y-cordinate*/, (cell_size + 15) - 1 /*width*/, cell_size - 1 /*height*/, /*color*/ colors[Cell_value]);
...
int main()
{
    ...
    Color black = {0 /*red*/, 0 /*green*/, 0 /*blue*/, 255 /*intensity*/}; // struct by raylib for colouring window

    InitWindow(600, 900, "Tetris Game"); // Initializes a Window
    SetTargetFPS(60);                    // setting frames per second(game speed set)

    game Game;

    while (!WindowShouldClose())
    { // for closing window if user input esc button

        Game.handling_input(); // for getting user input

        if (update(0.25))
        { // for block automatically move down towards but after certain specific interval
            Game.Moving_down();
        }

        BeginDrawing(); // creating blank canvas(surface for drawing)

        ClearBackground(black); // clearing background with clack colour
